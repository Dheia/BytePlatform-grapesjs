export class ComponentModule {
      ...
      .then(async (response) => {
        if (response.status == 200) {
          ...
          if (!data.error_code) {
            ...
            this.manager.addError(data.error, "byteplatform::component", {
              toEl,
              key,
              data,
            });
  ...
  clickEventComponent(e) {
    ...
    let strComponent = elCurrentTarget.getAttribute("byteplatform:component");
    let targetTo = elCurrentTarget.getAttribute("byteplatform:component-target");
    if (targetTo) {
      try {
