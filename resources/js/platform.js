...
export class PlatformModule extends PlatformEvent {
  ...
  doTrigger(el) {
    if (el && !el.__deleting) {
      this.dispatch("byteplatform::trigger_before", el);
      this.dispatch("byteplatform::trigger", el);
      this.dispatch("byteplatform::trigger_after", el);
    }
  }
  ...
  addMessage(message, type, component = "byteplatform", meta = {}) {
    this.dispatch("byteplatform::message", {
      message,
      type,
      component,
      meta,
  ...
  loading() {
    ...
    self.dispatch("byteplatform::loaded", self);
